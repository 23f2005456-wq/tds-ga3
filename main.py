...
async def chat(messages, model=None, max_tokens=800, force_json=True, retries=4):
    ...
    requested_model = model or config.TEXT_MODEL
    models_to_try = [requested_model]
    if requested_model != config.TEXT_MODEL:
        models_to_try.append(config.TEXT_MODEL)
            ...
            for use_json_mode in json_modes:
                ...
                for attempt in range(retries):
                    try:
                        ...
                        last_err = (
                            f"request error using {current_model}, "
                            f"json_mode={use_json_mode}: {type(e).__name__}: {e}"
                        )
                    ...
                    if r.status_code in (429, 500, 502, 503, 504):
                        last_err = (
                            f"HTTP {r.status_code} using {current_model}, "
                            f"json_mode={use_json_mode}: {r.text[:300]}"
                        )
                        await asyncio.sleep(1.5 * (attempt + 1))
                        continue
                    ...
                    if r.status_code >= 400:
                        last_err = (
                            f"HTTP {r.status_code} using {current_model}, "
                            f"json_mode={use_json_mode}: {r.text[:500]}"
                        )
                        # Try again without response_format, or with fallback model.
                        break
                    ...
                    try:
                        ...
                        last_err = (
                            f"bad response using {current_model}, "
                            f"json_mode={use_json_mode}: {type(e).__name__}: {e}; "
                            f"body={r.text[:500]}"
                        )
                        break
    ...
    raise RuntimeError(f"chat failed after all fallbacks: {last_err}")
...
async def answer_image(request: Request):
    ...
    # Accept both plain base64 and full data URLs.
    mime = "image/png"
    ...
    # Fix missing base64 padding and detect the true image type from magic bytes.
    try:
        ...
        last_debug_info = {"q2_error": f"invalid image base64: {e}"}
        return {"answer": ""}

    data_url = f"data:{mime};base64,{raw_b64}"

    first_prompt = (
        "Read the supplied image with extreme care and answer the question exactly.\n"
        "The image may be a chart, receipt, invoice, table, bar graph, line graph, "
        "pie chart, or document.\n"
        "First transcribe every relevant label and number. Then calculate only what "
        "the question asks. Recheck every digit and arithmetic operation.\n"
        "Do not estimate values that are printed explicitly.\n"
        "Return valid JSON with exactly these keys:\n"
        "{\"work\":\"brief transcription and calculation\", \"answer\":\"final answer\"}\n"
        "For a numeric answer, answer must contain ONLY the bare number, without "
        "currency symbols, commas, percent signs, units, or words. Preserve required decimals.\n"
        "For a text answer, copy the exact label from the image.\n\n"
        f"QUESTION: {question}"
    )
    ...
    for model in [config.VISION_MODEL, config.TEXT_MODEL]:
        for force_json in [True, False]:
            try:
                ...
                first = parse_json(raw)
                if first.get("answer") not in (None, ""):
                    break
            except Exception as e:
                errors.append(f"{model}/json={force_json}: {type(e).__name__}: {str(e)[:180]}")
        if first.get("answer") not in (None, ""):
            break

    candidate = normalize_answer(first.get("answer", ""))

    audit_prompt = (
        "Independently re-read the image and verify the proposed answer below. "
        "Do not trust it. Re-transcribe the relevant values, repeat the arithmetic, "
        "and correct the answer if needed.\n"
        "Return JSON with exactly these keys:\n"
        "{\"work\":\"brief independent verification\", \"answer\":\"final verified answer\"}\n"
        "Numeric answer: bare number only. Text answer: exact image label.\n\n"
    ...
    if candidate:
        for force_json in [True, False]:
            try:
                ...
                verified = parse_json(raw2)
                if verified.get("answer") not in (None, ""):
                    break
            except Exception as e:
                errors.append(
                    f"audit/{config.VISION_MODEL}/json={force_json}: "
                    f"{type(e).__name__}: {str(e)[:180]}"
                )

    final_answer = normalize_answer(
        verified.get("answer", candidate) if verified else candidate
    )

    last_debug_info = {
        "q2_question": question,
        "q2_mime": mime,
        "q2_first": first,
        "q2_candidate": candidate,
        "q2_verified": verified,
        "q2_final": final_answer,
        "q2_errors": errors,
    }
...
async def extract(request: Request):
    ...
    try:
        out = parse_json(await chat([{"role": "user", "content": prompt}],
                                    model=config.TEXT_MODEL, max_tokens=1200))
    except Exception:
        out = {}
...
async def answer_audio(request: Request):
    ...
    try:
        ...
        raw_llm = await chat([{"role": "user", "content": prompt}], model=config.TEXT_MODEL, max_tokens=1500)
